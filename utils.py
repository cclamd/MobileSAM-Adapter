...
def vis_image(imgs, pred_masks, gt_masks,net: nn.Module, save_path,reverse = False, points = None):
    ...
    # pred_masks_numpy = pred_masks.cpu().numpy()
    # np.save(save_path+'.npy', pred_masks_numpy)
    ...
    if c == 2:
        ...
        imgs = torchvision.transforms.Resize((1536,2048))(imgs)
        if imgs.size(1) == 1:
    ...
    masks = net.postprocess_masks(pred_masks.cpu(),
                                                              (1024, 1024),
                                                              (1536, 2048))[0].squeeze().detach().cpu().numpy()
        
        
    def show_mask(mask_, random_color=False):
        if random_color:
        ...
        new_array=(mask_image).astype(np.uint8)
        # new_array[new_array < 100] = 0
        new_array[new_array > 50] = 255
        # Convert the mask_array numpy array to a PIL.Image object
        mask_pil = Image.fromarray(new_array)
    ...
    # vutils.save_image(masks, fp=save_path, nrow=row_num, padding=10)
    vutils.save_image(tup, fp = save_path+".jpg", nrow = row_num, padding = 10)
